...
const unitData = {
    ...
    "Auxillary Directorate": [], "Budget Management Section": [], "CAFSD": [], "CAS": [],
    "CASAT": [], "CBEA": [], "CCIS": [], "CHS": [], "CIT": [], "COE": [], "COL": [],
    "COM": [], "CTE": [], "CVM": [], "Cash Management": [], "Distance Learning Office": [],
    "ETEEAP": [], "Extension Directorate": [], "General Services Directorate": [],
    "Graduate School": [], "Health and Wellness Services": [], "Human Resources Management Office ": [],
    "Information Technology Center": [], "Innovation and Technology Directorate (former S&T Park)": [],
    "Instructional Materials Development Office": [], "Internal Audit Services": [],
    "Internationalization, Linkages, and Partnership Directorate": [], "NBERIC": [],
    "OJT - SIPP Practicum Coordinating Office": [], "Office of the University and Board Secretary": [],
    "PPDO": [], "Planning Office": [], "Procurement Division": [], "Quality Assurance": [],
    "Records and Archives Management Office": [], "Research Directorate": [],
    "Strategic Communication Office": [], "Students and Affairs Services Office": [],
    "Supply and Property Management Office": [], "URERB": [], "University Library System": [],
...
const props = defineProps({
    ...
    filters: {
        type: Object,
        default: () => ({ search: '', unit: '', sort_sentiment: '', year: '' })
    },
    latestBatch: {
        type: Object,
        default: () => ({}) 
...
// Initialize yearFilter. If no year in filters, default to "All Years" (empty string)
const yearFilter = ref(props.filters?.year || ''); 
const sortSentiment = ref(props.filters?.sort_sentiment || '');
const isRefreshing = ref(false);

// --- 3. BATCH PROGRESS LOGIC ---
...
const checkAnalysisStatus = async () => {
    ...
    } catch (e) { /* Silently fail if no active batch */ }
};
...
const performSearch = debounce(() => {
    router.get(window.location.pathname, { 
        ...
        // Send 'All Years' if empty to match Controller logic or send the specific year
...
const toggleSort = () => {
    if (sortSentiment.value === '') sortSentiment.value = 'desc'; 
    else if (sortSentiment.value === 'desc') sortSentiment.value = 'asc';  
    else sortSentiment.value = '';     
    performSearch();
};

// Watch all filters for automatic searching
watch([search, unitFilter, yearFilter], () => performSearch());
...
const refreshData = () => {
    ...
    router.reload({ 
        only: ['feedback', 'stats', 'topPerformers', 'needsImprovement', 'availableYears'],
        ...
        <div class="dashboard-container">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-5 mb-8">
                <div v-for="(item, index) in [
                    ...
                    :class="item.color">
                    <div class="text-base opacity-90 uppercase tracking-[0.1em] font-black leading-tight">{{ item.label }}</div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-12 items-start">
                <div class="bg-gradient-to-br from-[#0C4B05] to-[#1a6e4d] rounded-[35px] p-8 shadow-sm text-white border-b-8 border-[#FFCD00]">
                     ...
                     <div class="space-y-6">
                        ...
                        <button v-if="props.topPerformers.length > 3" @click="showAllTop = !showAllTop"
                ...
                <div class="bg-white rounded-[35px] p-8 shadow-sm border border-red-50 border-b-8 border-red-500">
                     ...
                     <div class="space-y-6">
                        ...
                        <button v-if="props.needsImprovement.length > 3" @click="showAllNeedsImprovement = !showAllNeedsImprovement"
            ...
            <div class="bg-white rounded-[25px] p-8 shadow-sm border border-gray-100 font-sans">
                ...
                <div class="flex flex-col md:flex-row gap-4 mb-4">
                    <div class="relative flex-grow md:max-w-xs">
                        ...
                        <input v-model="search" type="text" placeholder="Search feedback..." 
                            class="w-full bg-gray-50 border-none text-gray-700 text-sm pl-10 pr-4 py-3 rounded-xl focus:ring-1 focus:ring-[#0c4b33] font-medium">
                    </div>

                    <div class="relative w-full md:w-44">
                        <select v-model="yearFilter" class="appearance-none w-full bg-gray-50 border border-gray-100 text-gray-700 text-sm font-bold py-3 pl-4 pr-10 rounded-xl focus:ring-1 focus:ring-[#0c4b33] cursor-pointer">
                            <option value="">All Years</option>
                            <option v-for="year in props.availableYears" :key="year" :value="year">{{ year }}</option>
                        </select>
                    ...
                    <div class="relative w-full md:w-72">
                        <select v-model="unitFilter" class="appearance-none w-full bg-gray-50 border border-gray-100 text-gray-700 text-sm font-bold py-3 pl-4 pr-10 rounded-xl focus:ring-1 focus:ring-[#0c4b33] cursor-pointer truncate">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="text-gray-400 text-xs font-black uppercase tracking-widest border-b border-gray-100">
                                <th class="py-4 pr-4 pl-4">Year</th> 
                                <th class="py-4 pr-4">Operating Unit</th>
                                <th class="py-4 pr-4 w-1/3">Review</th>
                                <th class="py-4 pr-4">Services</th>
                                <th class="py-4 pr-4">Theme</th>
                                <th class="py-4 text-right pr-4 cursor-pointer hover:text-[#0c4b33] select-none" @click="toggleSort">
                        ...
                        <tbody class="divide-y divide-gray-50 text-sm">
                            <tr v-for="row in props.feedback.data" :key="row.id" class="group hover:bg-gray-50/80 transition-colors">
                                <td class="py-5 pr-4 pl-4 align-top font-black text-[#0c4b33]">{{ row.year }}</td>
                                ...
                                <td class="py-5 pr-4 text-gray-600 text-sm leading-relaxed group-hover:text-gray-900 align-top italic">
                                    "{{ row.feedback_text }}"
                                </td>
                                <td class="py-5 pr-4 align-top">
                                ...
                                <td class="py-5 pr-4 align-top">
                                    <span v-if="row.topic" class="inline-flex items-center px-2 py-1 rounded bg-indigo-50 text-indigo-700 border border-indigo-200 text-xs font-black uppercase tracking-wider shadow-sm">
                                        {{ row.topic }}
                                    </span>
                                    <span v-else class="text-xs text-gray-300 font-mono font-bold">---</span>
                                </td>
                                <td class="py-5 text-right align-top pr-4">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-black uppercase tracking-wide border"
                                        :class="{
                            ...
                            <tr v-if="props.feedback.data.length === 0">
                                <td colspan="6" class="py-12 text-center text-gray-400 italic">No data found matching your filters.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div class="py-5 border-t border-gray-50 flex justify-between items-center mt-2">
                    <div class="text-sm text-gray-500 font-bold uppercase tracking-wide">
                        <span v-if="props.feedback.total > 0">Page {{ props.feedback.current_page }} of {{ props.feedback.last_page }}</span>
                        <span v-else>No Pages</span>
                    </div>
                    <div class="flex gap-2">
                        <Link v-if="props.feedback.prev_page_url" :href="props.feedback.prev_page_url" :preserve-scroll="true" :preserve-state="true"
                            class="px-5 py-2.5 bg-white border border-gray-200 text-gray-600 rounded-lg text-sm font-bold hover:bg-[#0c4b33] hover:text-white transition shadow-sm">
                            Previous
                        </Link>
                        <Link v-if="props.feedback.next_page_url" :href="props.feedback.next_page_url" :preserve-scroll="true" :preserve-state="true"
                            class="px-5 py-2.5 bg-white border border-gray-200 text-gray-600 rounded-lg text-sm font-bold hover:bg-[#0c4b33] hover:text-white transition shadow-sm">
                            Next
                        </Link>
                    </div>
